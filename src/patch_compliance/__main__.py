...
def main():
    ...
    for x in os_whitelist:
        valid += "'" + x + "',"
    valid = valid[:-1]
    parser.add_argument("-f", "--file", help="Name of the host file containing line seperated list of IPs or FQDNs. Defaults to hosts.txt",required=False)
    ...
    audit = patch_compliance.PatchCompliance(host_file=args.file, os_list=os_final)
    audit.run()
